...
2. Under **Settings -> Environment Variables**, set `ANTHROPIC_API_KEY`, `NEXT_PUBLIC_SUPABASE_URL`, the Supabase publishable key, and `DOSSIER_ALLOWED_EMAILS`. The `NEXT_PUBLIC_*` values are inlined at build time, so set them *before* the build you intend to ship and redeploy after changing them.
